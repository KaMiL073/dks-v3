...
const collectionName = 'schooling';
...
//   linkPath.shift();
  const formName = 'Schooling';
//   const nameSchooling = linkPath[1];
  const nameSchooling = 'KIP-modele-8';
  ...
  const fieldElement = fields.map((field) => (
    <div key={field.name}><FieldElement dataField={field} event={nameSchooling} register={register} /></div>
  ));
