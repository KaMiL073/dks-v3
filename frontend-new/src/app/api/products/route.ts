...
import { getFiltersMeta, type FilterField } from "@/lib/productFiltersMeta";
...
const readItemsUnsafe = readItems as unknown as (collection: unknown, query?: unknown) => unknown;
...
function extractValuesFromItemValue(val: unknown): string[] {
  ...
  if (Array.isArray(val)) {
    return val.flatMap((x) => extractValuesFromItemValue(x));
  }
...
export async function GET(request: Request) {
  try {
    ...
    const allProductsUnknown = await client.request(
      readItemsUnsafe("products", {
        fields: [
          "id",
          "model",
          "slug",
    ...
    const allProducts = Array.isArray(allProductsUnknown) ? (allProductsUnknown as Product[]) : [];
    ...
    const productsWithFilters = await Promise.all(
      paginated.map(async (product) => {
        try {
          const filtersMeta = await getFiltersMeta(product);
          return {
            ...product,
            filtersMeta,
          };
    ...
    return NextResponse.json({
      ...
      total,
      totalPages,
      count: productsWithFilters.length,
    ...
    console.error("❌ Błąd w /api/products:", message, error);

    return NextResponse.json(
      { error: "Products endpoint failed", message },
      { status: 502 }
    );
  }
}
...
// // ✅ wrapper: bez any, TS przestaje marudzić o directus.request i readItems("products"...)
// const client = directus as unknown as { request: (op: unknown) => Promise<unknown> };
// const readItemsUnsafe = readItems as unknown as (collection: unknown, query?: unknown) => unknown;
...
//     // bazowy filtr (Directus filter DSL)
//     const baseFilter: Record<string, unknown> = {
//       status: { _eq: "published" },
//       _and: [
//         {
...
//     // ✅ zamiast directus.request(readItems(...)) -> wrapper
//     const allProductsUnknown = await client.request(
//       readItemsUnsafe("products", {
//         fields: [
//           "id",
...
//     console.error("❌ Błąd w /api/products:", message, error);
//     return NextResponse.json({ error: "Products endpoint failed", message }, { status: 502 });
//   }
// }
// import { NextResponse } from "next/server";
// import { directus } from "@/lib/directus";
