...
async function getPromos(): Promise<Offer[]> {
  try {
    const data = (await directus.request(
      readSingleton("general_settings" as never, {
        fields: ["promos_widget"],
      } as never)
    )) as {
      promos_widget?: DirectusPromo[] | null;
    };

    return (data.promos_widget ?? [])
      .filter((item) => item.active)
      .map((item) => ({
        title: item.label,
        subtitle: item.model?.trim() ?? "",
        image: item.image
          ? `/backend/assets/${item.image}?imwidth=1920`
          : undefined,
        price: item.price ? `od ${item.price} zł netto m/c` : "",
        link: item.link?.replace("https://dks.pl", "") ?? "#",
        description: item.description ?? "",
        highlight: Boolean(item.highlighted),
      }));
  } catch (error) {
    console.error("PromoSection Directus error:", error);
    return [];
  }
}
    ...
    >
      ...
      <div className="flex-1 px-6 md:px-10 py-6 md:py-10 flex flex-col items-center justify-between gap-4 md:gap-6">
        ...
        <div className="text-center mt-auto w-full">
          {offer.price && (
            <span className="block text-base md:text-lg font-semibold mb-2">
              {offer.price}
            </span>
          )}
          ...
          <div
            className="
              text-xs md:text-base leading-snug text-left
              [&_ul]:list-disc
              [&_ul]:pl-5
              [&_ul]:mt-2
              [&_li]:mb-1
  ...
  return (
    <section className="px-6 md:px-6 2xl:px-28 py-12 md:py-20 flex flex-col gap-8 md:gap-16">
      ...
      <div className="hidden 2xl:grid grid-cols-3 gap-10 items-stretch justify-items-center">
        {offers.map((offer, idx) => (
          <OfferCard key={`${offer.title}-${idx}`} offer={offer} />
        ))}
      </div>
    </section>
  );
...
//   title: string;
//   subtitle: string;
//   image?: string; // może być absolutny URL albo względny
//   price: string;
//   link: string;
//   description: string;
//   highlight?: boolean;
...
// // Zamienia absolutne URL-e Directusa na względne /backend/assets/...
// function normalizeDirectusImage(src?: string) {
//   if (!src) return null;
...
//     }
//     return src; // np. placeholdery zostaw
//   } catch {
//     return src;
//   }
// }
...
// function OfferCard({ offer }: { offer: Offer }) {
//   const imgSrc = normalizeDirectusImage(offer.image);

//   // Jeśli obraz pochodzi z /backend/assets — daj unoptimized, żeby NIE było /_next/image
//   const isDirectusAsset = Boolean(imgSrc?.startsWith("/backend/assets/"));
...
//             }`}
//             aria-label={`Przejdź do oferty ${idx + 1}`}
//           />
//         ))}
//       </div>
