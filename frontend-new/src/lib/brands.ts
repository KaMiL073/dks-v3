// frontend-new/src/lib/brands.ts
...
export async function getBrandBySlugOrName(
  slugOrName: string
): Promise<Brand | null> {
  try {
    const bySlug = (await directus.request(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (readItems as any)("brands", {
    ...
    console.warn(
      "⚠ getBrandBySlugOrName: slug zapytanie nieudane -> fallback po name",
      e
    );
  }
