"use client";

import Image from "next/image";
import { setAttr } from "@/lib/visual-editor";
import Heading, { type HeadingTag } from "@/components/ui/Typography/Heading";
import Button from "../ui/Button";

interface RichContentItem {
  id: string | number;
  title?: string;

  header_type?: unknown;
  heading_styles?: unknown;

  subtitle?: string;
  subtitle_type?: unknown;
  subtitle_styles?: unknown;

  content?: string;
  image?: string;

  layout?: "text_left" | "text_right" | string;

  text_button?: string;
  url_button?: string;

  [key: string]: unknown;
}

const HEADING_TAGS: HeadingTag[] = ["h1", "h2", "h3", "h4", "h5", "h6"];

const HEADING_STYLE_KEYS = [
  "h1_normal",
  "h1_semibold",
  "h1_underline",

  "h2_normal",
  "h2_semibold",
  "h2_underline",

  "h3_normal",
  "h3_semibold",
  "h3_underline",

  "h4_normal",
  "h4_semibold",
  "h4_underline",
  "h4_italic",

  "h5_normal",
  "h5_semibold",
  "h5_underline",
  "h5_italic",

  "h6_normal",
  "h6_semibold",
  "h6_underline",
] as const;

type HeadingStyleKey = (typeof HEADING_STYLE_KEYS)[number];

function isHeadingTag(v: unknown): v is HeadingTag {
  return typeof v === "string" && (HEADING_TAGS as string[]).includes(v);
}

function isHeadingStyleKey(v: unknown): v is HeadingStyleKey {
  return (
    typeof v === "string" &&
    (HEADING_STYLE_KEYS as readonly string[]).includes(v)
  );
}

function asString(v: unknown): string | null {
  if (typeof v === "string") return v;

  if (typeof v === "number") return String(v);

  if (v && typeof v === "object") {
    const obj = v as Record<string, unknown>;

    return (
      asString(obj.value) ??
      asString(obj.key) ??
      asString(obj.name) ??
      asString(obj.title) ??
      null
    );
  }

  return null;
}

function normalizeHeadingStyle(
  value: unknown,
  fallback: HeadingStyleKey
): HeadingStyleKey {
  const raw = asString(value);

  if (isHeadingStyleKey(raw)) return raw;

  return fallback;
}

function headingTagFromStyle(value: HeadingStyleKey): HeadingTag {
  const tag = value.split("_")[0];

  return isHeadingTag(tag) ? tag : "h2";
}

function normalizeHeadingTag(
  typeValue: unknown,
  styleValue: HeadingStyleKey,
  fallback: HeadingTag
): HeadingTag {
  const rawType = asString(typeValue);

  if (isHeadingTag(rawType)) return rawType;

  return headingTagFromStyle(styleValue) ?? fallback;
}

export default function RichContentBlock({
  item,
}: {
  item: RichContentItem;
}) {
  const imageUrl = item?.image
    ? `/backend/assets/${item.image}`
    : "/static/placeholder-product.svg";

  const isTextRight = item.layout === "text_right";

  const isDirectus = imageUrl.startsWith("/backend/assets/");

  const headerHeadingValue = normalizeHeadingStyle(
    item.heading_styles,
    "h2_normal"
  );

  const subtitleHeadingValue = normalizeHeadingStyle(
    item.subtitle_styles,
    "h3_normal"
  );

  const headerTag = normalizeHeadingTag(
    item.header_type,
    headerHeadingValue,
    "h2"
  );

  const subtitleTag = normalizeHeadingTag(
    item.subtitle_type,
    subtitleHeadingValue,
    "h3"
  );

  return (
    <section
      className="bg-surface-page flex flex-col justify-center items-start gap-6 py-18 px-4 sm:px-6 lg:px-8 xl:px-28"
      data-directus={setAttr({
        collection: "rich_content",
        item: String(item.id),
        fields: ["layout"],
        mode: "popover",
      })}
    >
      <div
        className={`flex flex-col md:flex-row ${
          isTextRight ? "md:flex-row" : "md:flex-row-reverse"
        } items-start md:items-center justify-between gap-8 md:gap-12 w-full`}
      >
        <div className="w-full md:w-1/2">
          <Image
            src={imageUrl}
            alt={item.title || "Obrazek"}
            width={576}
            height={629}
            className="w-full h-auto object-cover rounded-lg"
            unoptimized={isDirectus}
            data-directus={setAttr({
              collection: "rich_content",
              item: String(item.id),
              fields: ["image"],
              mode: "modal",
            })}
          />
        </div>

        <div
          className="w-full md:w-1/2 flex flex-col justify-center items-start gap-6 py-4"
          data-directus={setAttr({
            collection: "rich_content",
            item: String(item.id),
            fields: [
              "title",
              "header_type",
              "heading_styles",
              "subtitle",
              "subtitle_type",
              "subtitle_styles",
              "content",
            ],
            mode: "modal",
          })}
        >
          <div className="flex flex-col justify-start items-start gap-9 w-full">
            {item.title && (
              <div
                className="w-full"
                data-directus={setAttr({
                  collection: "rich_content",
                  item: String(item.id),
                  fields: ["title", "header_type", "heading_styles"],
                  mode: "popover",
                })}
              >
                <Heading
                  as={headerTag}
                  headingValue={headerHeadingValue}
                >
                  {item.title}
                </Heading>
              </div>
            )}

            {item.subtitle && (
              <div
                className="w-full"
                data-directus={setAttr({
                  collection: "rich_content",
                  item: String(item.id),
                  fields: [
                    "subtitle",
                    "subtitle_type",
                    "subtitle_styles",
                  ],
                  mode: "popover",
                })}
              >
                <Heading
                  as={subtitleTag}
                  headingValue={subtitleHeadingValue}
                >
                  {item.subtitle}
                </Heading>
              </div>
            )}

            {item.content && (
              <div
                className="rich-content w-full"
                dangerouslySetInnerHTML={{ __html: item.content }}
                data-directus={setAttr({
                  collection: "rich_content",
                  item: String(item.id),
                  fields: ["content"],
                  mode: "modal",
                })}
              />
            )}
          </div>

          {item.text_button && (
            <div
              data-directus={setAttr({
                collection: "rich_content",
                item: String(item.id),
                fields: ["text_button", "url_button"],
                mode: "popover",
              })}
            >
              <Button
                href={item.url_button || "#"}
                className="mt-4"
              >
                {item.text_button}
              </Button>
            </div>
          )}
        </div>
      </div>
    </section>
  );
}