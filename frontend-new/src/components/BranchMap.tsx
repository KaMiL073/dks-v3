...
const BranchMap: React.FC<BranchMapProps> = ({ branch }) => {
  ...
  const salesPhone = salesContact?.phones?.[0] ?? "";
  const salesEmail = salesContact?.emails?.[0] ?? "";
  const servicePhone = serviceContact?.phones?.[0] ?? "";
  const serviceEmail = serviceContact?.emails?.[0] ?? "";
  ...
  return (
    <section className="py-20 w-full bg-surface-page overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-8 px-8 lg:px-28">
        ...
        <div className="flex-1 min-w-[220px]">
          <p className="text-Text-body text-xl font-semibold">
