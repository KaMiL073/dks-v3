...
type MatchInfo = { id: string; homeTeamId: string; awayTeamId: string; homeName: string; awayName: string; pitchId: string; groupId: string; startTime: string; endTime: string; homeScore: number; awayScore: number };
type View = "main" | "teams" | "time" | "pitch" | "group" | "score" | "delete";

export function MatchEditModal({ tournamentId, match, teams, pitches, groups, action, deleteAction }: { tournamentId: string; match: MatchInfo; teams: Option[]; pitches: Option[]; groups: Option[]; action: (data: FormData) => Promise<void>; deleteAction: (data: FormData) => Promise<void> }) {
  ...
  const titles: Record<View,string> = { main: `${match.homeName} × ${match.awayName}`, teams: "Drużyny", time: "Czas", pitch: "Boisko", group: "Grupa", score: "Wynik meczu", delete: "Usuwanie rozgrywki" };
  ...
  return <>
    ...
    {open && <div className="team-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section className={`team-modal match-edit-modal${view === "delete" ? " match-delete-modal" : ""}`} role="dialog" aria-modal="true" aria-labelledby="match-edit-title">
        ...
        {view === "main" ? <div className="match-edit-menu">
          {([['teams','Drużyny'],['time','Czas'],['pitch','Boisko'],['group','Grupa'],['score','Wynik']] as const).map(([key,label]) => <button type="button" key={key} onClick={() => setView(key)}><span>{label}</span><Image src="/dks-cup/figma/org/edit.svg" alt="" width={20} height={20}/></button>)}
          <div className="match-delete-trigger"><button className="match-delete-button" type="button" onClick={() => setView("delete")}>Usuń rozgrywkę</button></div>
        </div> : view === "delete" ? <div className="match-delete-confirmation">
          <p>Czy na pewno chcesz usunąć rozgrywkę {match.homeName} × {match.awayName}? Tej operacji nie da się cofnąć.</p>
          <form action={deleteAction}>
            <input type="hidden" name="tournamentId" value={tournamentId}/>
            <input type="hidden" name="matchId" value={match.id}/>
            <button className="match-delete-confirm" type="submit">Usuń</button>
            <button className="match-delete-cancel" type="button" onClick={() => setView("main")}>Anuluj</button>
          </form>
          ...
          {view === "score" && <div className="match-score-pair"><label><span>{match.homeName}</span><input name="homeScore" type="number" inputMode="numeric" min="0" max="999" defaultValue={match.homeScore} required/></label><b>:</b><label><span>{match.awayName}</span><input name="awayScore" type="number" inputMode="numeric" min="0" max="999" defaultValue={match.awayScore} required/></label></div>}
